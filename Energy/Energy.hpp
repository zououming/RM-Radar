//
// Created by bazinga on 19-10-29.
//

#ifndef RM_NBUT2020_ENERGY_HPP
#define RM_NBUT2020_ENERGY_HPP
...
void Energy::run()
{
    ...
    string config_file_name = "/home/shady/初稿修改/Settings/param_config.xml";
    Settings set_Energy(config_file_name);
    while(cap.read(image)) {
        ...
        threshold(image, image, set_Energy.Energy_MIN, set_Energy.Energy_MAX, THRESH_BINARY);        //阈值要自己调
        ...
        threshold(image, image, set_Energy.Energy_MIN, set_Energy.Energy_MAX, THRESH_BINARY_INV);
        ...
        for (size_t i = 0; i < contours.size(); i++) {
            ...
            if (area < set_Energy.Energy_areaMIN || set_Energy.Energy_areaMAX < area) continue;
            ...
            if (aim > set_Energy.Energy_aimMIN && aim < set_Energy.Energy_aimMAX) {
                for (int j = 0; j < 4; j++)
                {
